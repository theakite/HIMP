...
enum BACnetPropertyIdentifier {
    ...
    PROP_NO_PROPERTY = 4194304  // not standard, but we are using it!
};
typedef enum BACnetPropertyIdentifier BACNET_PROPERTY_ID;
...
typedef enum BACnetEngineeringUnits BACNET_ENGINEERING_UNITS;
...
enum BACnetObjectType {
    ...
    // the procedures and constraints described in Clause 23.
    OBJECT_RESERVED_0 = 23,     // FIXME: keep updated
    OBJECT_PROPRIETARY_START = 128,
    OBJECT_PROPRIETARY_END = 1023,
    OBJECT_NONE = 1024
};
typedef enum BACnetObjectType BACNET_OBJECT_TYPE;
...
typedef enum BACnetAbortReason BACNET_ABORT_REASON;
...
typedef enum BACnetRejectReason BACNET_BACNET_REJECT_REASON;
...
typedef enum BACnetErrorClass BACNET_ERROR_CLASS;
...
typedef enum BACnetErrorCode BACNET_ERROR_CODE;
